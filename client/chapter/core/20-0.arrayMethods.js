...
const map = people.map((user) => user.age)
...
// cardTag.forEach(function(tag) {
//   ul.insertAdjacentHTML('beforeend', tag);
// });



cardTag.forEach((element, index, array) => {
  // 각 요소에 대해 실행할 코드
});
...
const resultAge = people.reduce((accumulator, user) => {
  return accumulator + user.age
}, 0); // ⭐️ 초기값 중요 ㅠㅠ 안쓰면 이상한거 나와
